...
$('document').ready(function () {
    ...
    var NPSContentBuilderMethods = {
      ...
      alerts: function (element, cat) { //passing in the element(object returned by the API call) and the cat(category)
        ...
        //VAR HEADERS
        var header = buildCardHeader(category);   
        var header2 = buildCardHeader(title);  

        //VAR INNERS
        var inner = buildCardInner(description);

        //VAR LINKS
        var link = buildCardLink(url);
    ...
    //HEADER
    $card.prepend(header);              //prepend category to card
    $card.prepend(header2);             //prepend title to card
    
    //INNER
    $card.prepend(inner);               //prepend description to card

    //LINKS
    $card.prepend(link);                //prepend url to card
    ...
    //END
      ...
      articles: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(listingDescription);
        var inner2 = buildCardInner(latLong);
        var inner3 = buildCardInner(relatedParks);
        var inner4 = buildCardInner(id);


        //VAR LINKS
        var link = buildCardLink(url);
        var link2 = buildCardLink(listingImage);
      ...
      <div class="card-body">
        ...
        `)
    //HEADER
    $card.prepend(header);              //prepend title to card
    
    //INNER
    $card.prepend(inner);               //prepend listingDescription to card
    $card.prepend(inner2);              //prepend latLong to card
    $card.prepend(inner3);              //prepend relatedParks to card
    $card.prepend(inner4);              //prepend id to card


    //LINKS
    $card.prepend(link);                //prepend url to card
    $card.prepend(link2);               //prepend listingimage to card
  ...
  <br>THERE ARE MANY MORE OF THESE`;
        ...
        var header2 = buildCardHeader(accessibility);

        //VAR INNERS
        var inner = buildCardInner(description);
        var inner2 = buildCardInner(directionsOverview);
        var inner3 = buildCardInner(latLong);
        var inner4 = buildCardInner(amenities);
        var inner5 = buildCardInner(campsites);
        var inner6 = buildCardInner(id);
        var inner7 = buildCardInner(parkCode);
        var inner8 = buildCardInner(regulationsOverview);
        var inner9 = buildCardInner(reservationsDescription);
        var inner10 = buildCardInner(reservationsSitesFirstCome);
        var inner11 = buildCardInner(reservationsSitesReservable);
        var inner12 = buildCardInner(weatherOverview);

        //VAR LINKS
        var link = buildCardLink(directionsUrl);
        var link2 = buildCardLink(regulationsUrl);
        var link3 = buildCardLink(reservationsUrl);
    ...
    //HEADER
    $card.prepend(header);              //prepend name to card
    $card.prepend(header2);             //prepend accessibility to card
    
    //INNER
    $card.prepend(inner);               //prepend description to card
    $card.prepend(inner2);              //prepend directiionsOverview to card
    $card.prepend(inner3);              //prepend latLong to card
    $card.prepend(inner4);              //prepend amenities to card
    $card.prepend(inner5);              //prepend campsites to card
    $card.prepend(inner6);              //prepend id to card
    $card.prepend(inner7);              //prepend parkCode to card
    $card.prepend(inner8);              //prepend regulationsOverview to card
    $card.prepend(inner9);              //prepend reservationsDescription to card
    $card.prepend(inner10);             //prepend reservationsSitesFirstCome to card
    $card.prepend(inner11);             //prepend reservationsSitesReservable to card
    $card.prepend(inner12);             //prepend weatherOverview to card



    //LINKS
    $card.prepend(link);                //prepend directionsUrl to card
    $card.prepend(link2);               //prepend regulationsUrl to card
    $card.prepend(link3);               //prepend reservationsUrl to card
    ...
    //END
      ...
      parks: function (element, cat) {
        ...
        //VAR HEADERS
        var header = buildCardHeader(name);             
        var header2 = buildCardHeader(fullName);

        //VAR INNERS
        var inner = buildCardInner(description);
        var inner2 = buildCardInner(directionsInfo);
        var inner3 = buildCardInner(latLong);
        var inner4 = buildCardInner(states);
        var inner5 = buildCardInner(weatherInfo);

        //VAR LINKS
        var link = buildCardLink(directionsUrl);
        var link2 = buildCardLink(url);
    ...
    `)
        //HEADER
        $card.prepend(header);              //prepend name to card
        $card.prepend(header2);             //prepend fullName to card

        //INNER
        $card.prepend(inner);               //prepend description to card
        $card.prepend(inner2);              //prepend directionsInfo to card
        $card.prepend(inner3);              //prepend latLong to card
        $card.prepend(inner4);              //prepend states to card
        $card.prepend(inner5);              //prepend weatherInfo to card


        //LINKS
        $card.prepend(link);                //prepend directionsUrl to card
        $card.prepend(link2);               //prepend Url to card


        //CARD
        $content.prepend($card);            //prepend card to content
        //END
      },
      ...
      events: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(abstract);
        var inner2 = buildCardInner(dates);
        var inner3 = buildCardInner(feeInformation);
        var inner4 = buildCardInner(id);
        var inner5 = buildCardInner(image);
        var inner6 = buildCardInner(location);
        var inner7 = buildCardInner(parkCode);
        var inner8 = buildCardInner(recurrence);
        var inner9 = buildCardInner(time);
  ...
  `)
        ...
        //INNER
        $card.prepend(inner);               //prepend abstract to card
        $card.prepend(inner2);              //prepend dates to card
        $card.prepend(inner3);              //prepend feeInformation to card
        $card.prepend(inner4);              //prepend id to card
        $card.prepend(inner5);              //prepend image to card
        $card.prepend(inner6);              //prepend location to card
        $card.prepend(inner7);              //prepend parkCode to card
        $card.prepend(inner8);              //prepend recurrence to card
        $card.prepend(inner9);              //prepend time to card
      ...
      lessonplans: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(commonCore);
        var inner2 = buildCardInner(gradeLevel);
        var inner3 = buildCardInner(questionObjective);
        var inner4 = buildCardInner(id);
        var inner5 = buildCardInner(subject);
        var inner6 = buildCardInner(duration);
        var inner7 = buildCardInner(parks);
        ...
        //INNER
        $card.prepend(inner);               //prepend commonCore to card
        $card.prepend(inner2);              //prepend gradeLevel to card
        $card.prepend(inner3);              //prepend questionObjective to card
        $card.prepend(inner4);              //prepend id to card
        $card.prepend(inner5);              //prepend subject to card
        $card.prepend(inner6);              //prepend duration to card
        $card.prepend(inner7);              //prepend parks to card
      ...
      newsreleases: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(abstract);
        var inner2 = buildCardInner(parkCode);
        var inner3 = buildCardInner(releaseDate);
        var inner4 = buildCardInner(id);
        ...
        //VAR LINKS
        var link = buildCardLink(url);
        var link2 = buildCardLink(imageURL);
        ...
        //HEADER
        $card.prepend(header);              //prepend title to car
        //INNER
        $card.prepend(inner);               //prepend abstrat to card
        $card.prepend(inner2);              //prepend parkCode to card
        $card.prepend(inner3);              //prepend releaseDate to card
        $card.prepend(inner4);              //prepend id to card


        //LINKS
        $card.prepend(link);                //prepend Url to card
        $card.prepend(link2);                //prepend ImageURL to card
      ...
      people: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(listingImageCredit);
        var inner2 = buildCardInner(listingImageAltText);
        var inner3 = buildCardInner(listingImageTitle);
        var inner4 = buildCardInner(listingImageCaption);
        var inner5 = buildCardInner(relatedParks);

        //VAR LINKS
        var link = buildCardLink(url);
        var link2 = buildCardLink(listingImageUrl);
        ...
        //INNER
        $card.prepend(inner);               //prepend listingImageCredit to card
        $card.prepend(inner2);              //prepend listingImagealtText to card
        $card.prepend(inner3);              //prepend listingImageTitle to card
        $card.prepend(inner4);              //prepend listingImageCaption to card
        $card.prepend(inner5);              //prepend relatedParks to card

        //LINKS
        $card.prepend(link);                //prepend Url to card
        $card.prepend(link2);                //prepend listingImageUrl to card
      ...
      places: function (element, cat) {
        ...
        //VAR INNERS
        var inner = buildCardInner(listingDescription);
        var inner2 = buildCardInner(listingImageCredit);
        var inner3 = buildCardInner(listingImageAltText);
        var inner4 = buildCardInner(listingImageTitle);
        var inner5 = buildCardInner(listingImageCaption);
        var inner6 = buildCardInner(relatedParks);
        ...
        //INNER
        $card.prepend(inner);               //prepend listingDescription to card
        $card.prepend(inner2);              //prepend listingImageCredit to card
        $card.prepend(inner3);              //prepend listingImageAltText to card
        $card.prepend(inner4);              //prepend listingImageTitle to card
        $card.prepend(inner5);              //prepend listingImageCaption to card
        $card.prepend(inner6);              //prepend relatedParks to card
      ...
      visitorcenters: function (element, cat) {
        ...
        var inner3 = buildCardInner(latLong);
        var inner4 = buildCardInner(id);
        var inner5 = buildCardInner(directionsInfo);
        var inner6 = buildCardInner(contactsPhoneNumbers);
        var inner6 = buildCardInner(contactsEmailAddresses);


        //VAR LINKS
        var link = buildCardLink(url);
        var link2 = buildCardLink(directionsUrl);
        ...
        $card.prepend(inner3);              //prepend latLong to card
        $card.prepend(inner4);              //prepend id to card
        $card.prepend(inner5);              //prepend directionsInfo to card
        $card.prepend(inner6);              //prepend contactsPhoneNumbers to card
        $card.prepend(inner7);              //prepend contactsEmailAddresses to card



        //LINKS
        $card.prepend(link);                //prepend Url to card
        $card.prepend(link2);                //prepend directionsUrl to card
